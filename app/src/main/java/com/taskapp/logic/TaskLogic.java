...
public class TaskLogic {
    ...
    public void showAll(User loginUser) {
        ...
        tasks.forEach(task -> {
            ...
            String userName = loginUser.equals(task.getRepUser()) ? "あなた" : task.getRepUser().getName();
            System.out.println("%d. タスク名：%s, 担当者名：%sが担当しています, ステータス：%s".formatted(task.getCode(), task.getName(),
                    userName, status));
        });
    }
